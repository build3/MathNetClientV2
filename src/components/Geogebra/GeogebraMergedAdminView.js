...
class GeogebraMergedAdminView {
    constructor(params, workshopIds) {
        ...
        GeogebraMergedAdminView.registerApplet(this.params.id, this.ggbOnInit.bind(this));
        // eslint-disable-next-line no-undef
        this.appletContainer = new GGBApplet(this.params);
    }

    static initRegistry() {
        if (!GeogebraMergedAdminView.appletRegistry) {
            GeogebraMergedAdminView.appletRegistry = {};
        }
    }

    static registerApplet(appletId, initCallback) {
        GeogebraMergedAdminView.initRegistry();
        GeogebraMergedAdminView.appletRegistry[appletId] = initCallback;
    }

    static globalInitHandler(applet) {
        console.log(`Initializing ${applet}`);
        GeogebraMergedAdminView.initRegistry();
        if (GeogebraMergedAdminView.appletRegistry[applet]) {
            console.log(`${applet} has callbacks`);
            GeogebraMergedAdminView.appletRegistry[applet](applet);
        } else {
...
window.ggbOnInit = GeogebraMergedAdminView.globalInitHandler;
